...
import { updateIncidentServerAction } from "./update-incident.server";
...
const updateIncidentAction = async (incidentId: string, _: IncidentFormState, formData: FormData): Promise<IncidentFormState> => {
  ...
  return updateIncidentServerAction({
    incidentId,
    data: parsed.data,
    values,
  });
