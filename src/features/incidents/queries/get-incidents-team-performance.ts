...
const getIncidentsTeamPerformance =
  async (): Promise<IncidentsTeamPerformance> => {
    ...
    const { now, todayStart, last30DaysStart } = getRelativeDayBoundaries();
    ...
    return {
      period: {
        from: last30DaysStart.toISOString(),
        to: now.toISOString(),
      },
      resolvers: sortDesc(
